...
int main(void)
{
    ...
    while (true) {
        //struct rtio_sqe *sqe = rtio_sqe_acquire(&tof_rtio_ctx);
        //if (sqe == NULL) {
        //    printk("Failed to acquire SQE\n");
        //    return -1;
        //}
        ///* 2. Prepare the SQE as a "Read" onto our IO Device */
        //rtio_sqe_prep_read(sqe, &tof_iodev, RTIO_PRIO_NORM, NULL, 0, NULL);
        /* 3. Instruct RTIO to automatically use our Memory Pool for the raw buffer */
        //sqe->flags |= RTIO_SQE_MEMPOOL_BUFFER;
        /* 4. Fire the request! (This routes directly into `empty_sensor_submit`) */
        //rtio_submit(&sensor_rtio_ctx, 0);

        /////

        //struct rtio_cqe *cqe = rtio_cqe_consume_block(&tof_rtio_ctx);
        //if (cqe == NULL) {
        //    continue;
        //}
        ...
        LOG_INF("sensor_read_async_mempool done");
        ...
        LOG_INF("rtio_cqe_consume_block done");
        ...
        LOG_INF("rtio_cqe_get_mempool_buffer done");

		rtio_cqe_release(&tof_rtio_ctx, cqe);
        LOG_INF("rtio_cqe_release done");
        LOG_INF("------");
